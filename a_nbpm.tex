...
\equationref{eq_three_bpm} assumes that no error is present in the region between the involved BPMs.
...
In Figure~\ref{fig:hist1518} a comparison between Eqs.~(\ref{eq:pablo}) and~(\ref{eq:18}) is shown
...
\begin{figure}
	...
	\caption{TOP: Difference between the real (simulated)  horizontal $\beta$-functions and the ones
  calculated by \eqref{eq:pablo} in red and~(\ref{eq:18}) in blue, respectively. Data are from MADX
simulations of a lattice with quadrupolar errors only. BOTTOM: The same quantities are evaluated for
the case with additional magnet misalignments. rms denotes the root mean square of the deviations and
p-t-p denotes peak-to-peak deviation.}
	\label{fig:hist1518}
\end{figure}
...
\begin{wrapfigure}{O}[\figborderhang]{9cm}
	...
	\caption{Accuracy of the horizontal $\beta$-function evaluated via \eqref{eq:pablo} and~\eqref{eq:beta_g} with the effect of magnets and BPM misalignments taken into account. The accuracy of
...
ones simulated by MADX along with the ones inferred from \eqref{eq:pablo}, this time including
