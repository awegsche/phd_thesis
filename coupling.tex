...
\begin{chapterinfo}
    ...
    and the phase, as discussed in section~\ref{sec_comp}, and also has an impact on resonance driving terms. 
...
rescaling of the tune dependent denominators of the RDTs, this method will therefore be called the
\emph{rescaling} method.

The second one \cite{Miyamoto2010} applies the findings of a detailed study of the driven
particle motion using the equations of motion of the particle in the accelerator with an AC-dipole.
...
To consider beam motion that is driven \emph{and} coupled,
...
\end{align}
%
The following table summarises which quantities are analogous
\begin{center}
\begin{tabular}{ll}
    AC dipole & coupling \\
    \hline
    \hline
    $ A_\theta $        & $ J_{1,w}\sqrt{I_y\beta_y}$ \\
...
\begin{figure}
  ...
  \includestandalone{ac_kick_nf}
      \caption{
        Two AC-dipole kicks in $p_x$ at different positions in phase space can have completely
        different appearence in normal form coordinates. The transformation is not restricted to
        the two dimensions depictet in this figure but can happen in all four normal form coordinates
        $\zxp, \zxm, \zyp, \zym$.
      }
...
\begin{align}
    ...
    +\frac{A_\theta\beta(s_d)}{4\sin(\pi Q_z^-)} \Bigg\{&
...
\begin{align}
    \sum\limits_{T=0}^N R_x^T \Delta h_y^+(N-T)
        &=
        i\frac{A_\theta\beta(s_d)}{2} \left\{
...
\begin{align}
    ...
    \notag \\
    &= \frac{A_\theta\beta(s_d)}{4\sin(\pi Q_y^-)}
...
%
\begin{align}
    \four{h_y^{d+}}(Q_y^d) &= \frac{A_\theta\beta(s_d)}{4\sin\pi Q_y^-}
        \e{i\left[-\varphi_{s_ds}^{y} + \pi Q_y^-\text{sgn}(s-s_d)\right]}
    \komma
    \notag \\
    \four{h_y^{d-}}(Q_y^d) &= \frac{\lambda_y A_\theta\beta(s_d)}{4\sin\pi Q_y^-}
        \e{i\left[\varphi_{s_ds}^{y} + \pi Q_y^+\text{sgn}(s-s_d)\right]}
    \komma
    \notag \\
    \four{h_{y,x}^{+}}(Q_y^d) &= \frac{A_\theta\beta(s_d)}{8\sin\left[\pi (Q_y^d - Q_x)\right]}
...
\end{align}
%
From \eqref{eq_drv_f1001_final} it is apparent that the AC-dipole adds two effects to the difference RDT $f_{1001}$
the first one,
%
\begin{equation}
    f_{1001}^*(s_d) \frac{\sin\pi Q_y^-}{2\sin\left[\pi (Q_y^d-Q_x)\right]}
...
\end{equation}
%
is creating an additional source at the position of the AC-dipole, proportional to $f_{1001}$ at this position
and the second one,
%
\begin{equation}
    \lambda_y f_{1010}^*\e{2i\left[ \varphi_{s_ds}^{y} + \pi Q_y\right]}
...
\end{equation}
%
mixes the difference term with the sum term $f_{1010}$ (as well as a second contribution proportional to $f_{1001}$).
The mixing-in of those terms is
suppressed by the factors $\lambda_{y}$ and the denominator $\sin\left[\pi (\Qd{y} + Q_x)\right]$, respectively.
...
Measurements can only be performed on the real signal. The complex signal needed for the calculation
...
\begin{figure}[h]
  ...
  \caption{Comparison of the three methods with a coupling bump in the lattice.
    ...
    The formula method and new method (called \emph{new formula}) manage to reproduce well the $f_{1001}$.
...
\subsection{HL-LHC and stronger coupling}

The HL-LHC has more challenging optics and a similar coupling source as for the LHC simulations in the previous section
creates considerably larger coupling.

\begin{figure}
    \centering
    \includegraphics[width=.49\linewidth]{abs15}
    \hfill
    \includegraphics[width=.49\linewidth]{abs45}
    \caption{%
      These plots show the same comparison as Fig.~\ref{fig_comp_felix_ryo} but for the HL-LHC lattice
      at $\beta^*=\SI{15}{cm}$ with varying strengths of the skew quadrupoles which form the coupling
      bump. A degradation of the performance of the new formula is observed.
    }%
...
Two coupling strengths are tested. For reference, the initial setting for the first magnet in the bump is printed
in the top right corner of each plot%
\footnote{
  The final value depends on the matching but is still
  of the same order of magnitude.
}.
The left plot shows the same initial strength as for the LHC simulations in the previous section.
The matching of the closed coupling branch failed which resulted in non-zero coupling outside of the designated region.


Althoug still better than the conventional methods, the new formula does not achieve the same performance as before
and starts to lose its performance w.r.t.\ the other methods.
