...
\begin{document}
   \begin{tikzpicture} [axis/.style={->,black,thick}]
        ...
        %}
        \draw[axis] (-1,0) -- (10,0) node[anchor=west]{$\varphi\m(s)$};
        \draw[domain=1:8.5,variable=\x, samples=131,smooth] plot ({\x}, {sin(180.0*(\x-1.0))});
        \draw[domain=8.5:9.5,variable=\x, samples=131,smooth,dashed] plot ({\x}, {sin(180.0*(\x-1.0))});
        \draw[domain=0:1,variable=\x, samples=29,smooth] plot ({\x}, {sin(180.0*(\x-1.31))});
        \draw[domain=-1:0,variable=\x, samples=29,smooth,dashed] plot ({\x}, {sin(180.0*(\x-1.31))});
        \filldraw[black, fill=white] ($ (1, {sin(180.0*(1-1.31))}) $) circle (0.075);
        \quadrupole{(1,0)}{(A)};
        \node[above] at ($(A) + (-3*\bpmthickness,0.2)$) {$\delta K$};

        \coordinate (P1) at ($ (3.5, {sin(180.0*2.5)}) $);
        \coordinate (P2) at ($ (5.5, {sin(180.0*4.5)}) $);
        \draw[line width=.5mm, CLRemph2] ( 3.5, 0 ) -- (P1);
        \node[below] at (3.5,0) {BPM1};
        \filldraw[thick,fill=CLRemph2,draw=CLRemph2!70!black] (P1) circle (0.1);

        \draw[line width=.5mm, CLRemph2] ( 5.5, 0 ) -- (P2);
        \node[below] at (5.5,0) {BPM2};
