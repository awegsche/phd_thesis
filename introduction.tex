...
%
\begin{equation}
    H = \frac{k_1}{2}\left( x^2 - y^2 \right) + \frac{p_x^2+p_y^2}{2}
    \label{eq_hamil_quad}
\end{equation}
%
...
\cite{Georgi1982} and describe the particle motion going through an element in the lattice
with Hamiltonian $H$%
%
\begin{equation}
    x(s_b) = \e{\liemap{LH}}x(s_a)
...
Usually, non-linear terms are collected into a non-linear map $\e{\liemap{HL}}$, generated
by the non-linear Hamiltonian and a linear transfer mapt (rotation in CS-coordinates) $\mat{M}$:
%
\begin{equation}
    \mathscr{M}_w = \e{\liemap{LH_w}} \mat{M}_w
    \komma
\end{equation}
%
...
\begin{equation}
    \vec{z}(s_w + L) = \e{\liemap{LH_w}} \mat{M}_w \vec{z}(s_w)
\end{equation}
...
%
\begin{equation}
    \hat{p}_a = \frac{\hat{x}_b}{\cos\Delta} + \hat{x}_a \tan\Delta
\end{equation}
%
...
%
\begin{equation}
    h_z^\pm(s_a) = \hat{z}_a - i \left(\frac{\hat{z}_b}{\cos\Delta} + \hat{z}_a\tan\Delta \right) = \hat{z}_a(1-i\tan\Delta) - i \frac{\hat{z}_b}{\cos\Delta}
    \fstop
\end{equation}
%
