...
%
\begin{equation}
    h_z^\pm(s,N) = \sqrt{J_z} \e{\mp i \left[2N\pi Q_z + \varphi_z(s) + \varphi_{z,0}\right]}
\end{equation}
%
with $\varphi_{z,0}$ being an arbitrary initial phase.
...
\begin{equation}
    ...
    = A\sqrt{\beta_z(s)}\cos\left[ 2\pi N Q_z + \varphi_z(s) + \varphi_{z,0} \right]
...
%
\begin{equation}
    z(s,N) = A_d\sqrt{\beta_{z}^d(s)}\cos\left[ 2\pi N Q_z^d + \varphi_z^d(s) + \varphi_{z,0}^d \right]
    \fstop
...
are needed\footnote{Ryoichi Miyamoto, \emph{private communication}, Feb. 2017}.
First the transformation to physical space is performed while those terms in the phase that swap
sign between the two modes
are moved to the end:
%
\begin{align}
    z(s,N) = \frac{A_\theta\sqrt{\beta_z(s_d)\beta_z(s)}}{4\sin(\pi Q_z^-)}
        \Bigg[ 
            &\cos\left[2N\pi \Qd{z} + \Qd{z}\text{sgn}(s-s_d) + Q_z\text{sgn}(s-s_d)- \varphi_{s_ds}\right]
            \notag \\
            &- \lambda
            \cos\left[2N\pi \Qd{z} + \Qd{z}\text{sgn}(s-s_d) -Q_z\text{sgn}(s-s_d) +\varphi_{s_ds}\right]
        \Bigg]
...
\end{align}
%
Using sum and difference identities for trigonometric functions,
%
\begin{equation}
...
\begin{align}
    z(s,N) = \frac{A_\theta\sqrt{\beta_z(s_d)\beta_z(s)}}{4\sin(\pi Q_z^-)}
        \Bigg\{ 
            &(1-\lambda)\cos\left[2N\pi \Qd{z} + \Qd{z}\text{sgn}(s-s_d)\right] \cos\left[ Q_z\text{sgn}(s-s_d)- \varphi_{s_ds}\right]
            \notag \\
            &- (1+\lambda)
            \sin\left[2N\pi \Qd{z} + \Qd{z}\text{sgn}(s-s_d) \right] \sin\left[Q_z\text{sgn}(s-s_d) -\varphi_{s_ds}\right]
        \Bigg\}
        \fstop
\end{align}
%
A linear combination of oscillations with the same frequency forms a single oscillation, in this case 
%
\begin{equation}
    a\cos\\alpha - b\sin\alpha = \sqrt{a^2+b^2} \cos\left[\alpha + \tan(\frac{b}{a})]
    \komma
\end{equation}
%
with
%
\begin{align}
    a &= \frac{A_\theta \sqrt{\beta_z(s_d)\beta_z(s)}}{4\sin(\pi Q_z^-)} (1-\lambda) \cos(Q_z\text{sgn}(s-s_d)-\varphi_{s_ds}) \notag \\
    b &= \frac{A_\theta \sqrt{\beta_z(s_d)\beta_z(s)}}{4\sin(\pi Q_z^-)} (1+\lambda) \cos(Q_z\text{sgn}(s-s_d)-\varphi_{s_ds}) \notag \\
    \alpha &= \Qd{z}\left(2\pi N  + \text{sgn}(s-s_d)\right) 
    \fstop
\end{align}
%
This combined oscillation reads
%
\begin{equation}
    z(s,N) = \frac{A_\theta\sqrt{\beta_z(s_d)\beta_z(s)}}{4\sin(\pi Q_z^-)} \sqrt(1-2\lambda\cos[+\lambda^2)
\end{equation}
%