...
%
\begin{equation}
    \gamma(s)x^2(s) + 2\alpha(s)x(s)p_x(s) + \beta(s)p_x^2(s) = 2 J_x
\end{equation}
%
...
\end{equation}
%
where $p_x$ and $p_y$ are horizontal and vertical momentum, 
$\delta$ is deviation from the reference energy,
$\rho$ the bending radius of the reference frame,
$B_0$ the bending field and $A_s$ the electromagnetic potential.
...
The Hamiltonian for particle moving through a single quadrupole reads
%
\begin{equation}
    H = \frac{x\delta}{\rho} + \frac{1}{2}\left( x^2 - y^2 \right) + \frac{p_x^2+p_y^2}{2(1+\delta)}
    \label{eq_hamil_quad}
\end{equation}
%
In the simple example at hand, on-momentum particles are considered:
%
\begin{equation}
    H = \frac{1}{2}\left( x^2 - y^2 \right) + \frac{p_x^2+p_y^2}{2}
    \label{eq_hamil_quad}
\end{equation}
%
...
\begin{align}
    ...
    \Rightarrow \frac{\der^2 x}{\der s^2} &= \frac{\der p_z}{\der s} = -kx
\end{align}
...
\end{equation}
%
can be used to rewrite the Hamiltonian:
\begin{align}
    [x, H] = \frac{\partial H}{\partial p}\quad\text{ and }\quad
    [p, H] = -\frac{\partial H}{\partial x} 
    \fstop
\end{align}
%
The Poisson brackets form a \emph{Lie algebra} and therefore some properties can be utilised.
First, we define a \emph{Lie operator}
\begin{equation}
    \liemap{f}g = [f,g]
    \fstop
\end{equation}
...
\footnote{
    ...
    But the Lie map formalism is valid for the full thick lens description. The exponential
    map just becomes 
    $x(s_b) = \e{\liemap{LH^\text{thick}}}x(s_a)$ for a thick lens Hamiltonian.
}:
%
\begin{equation}
...
\end{equation}
%
In the linear case, $\mat{M}$ is a $4\times 4$ matrix and the indices
and for each of the four coordinates, a sum expression can be written
\begin{align}
    h_x^+(s_b) = m^1_{1} h_x^+(s_a) + \conj{m^1_{1}} h_x^-(s_a)
        + m^1_{2} h_y^+(s_a) + \conj{m^1_{2}} h_y^-(s_a) \notag \\
    h_x^-(s_b) = m^2_{1} h_x^+(s_a) + \conj{m^2_{1}} h_x^-(s_a)
        + m^2_{2} h_y^+(s_a) + \conj{m^2_{2}} h_y^-(s_a) 
\end{align}
and analogously for the vertical coordinates. The coefficients $m_c^r$ denote the matrix elements of
$\mat{M}$ at column $c$ and row $r$.
...
\end{equation}
%
where $n$ is the multipole order (1 for dipole, 2 for quadrupole etc)
...
\begin{equation}
\begin{matrix*}[l]
     \liemap{F} \zxp  &= \left[f_{0200} \zxmto{2}, \zxp \right]  &= 2i f_{0200} \zxm \komma\notag \\
     \liemapn{F}{2}\zxp &= \left[f_{2000} \zxpto{2}, 2i f_{0200} \zxm \right]
