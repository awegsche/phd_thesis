...
depicted in \figref{fig_frenet_serret}\index{Frenet-Serret coordinates} where the cartesian coordinates $\{x', y', z'\}$ are
...
\end{align}
%
\equationref{eq_3bpm_method} was developed in \cite{Castro1996} and used for LEP and in run I of LHC.
...
And secondly, a too large deviation from the optical axis inside a higher order magnet may lead to an undesirable
...
%
\begin{align}
    A^+_{0,n_y} &= \frac{H^+(0,n_y)}{\left|H^+(1,0)\right|} \notag \\
    B^+_{n_x,0} &= \frac{V^+(n_x,0)}{\left|V^+(0,1)\right|} 
    \fstop
\end{align}
%
...
%
\begin{align}
    A^+_{0,1}&=\frac{H^+(0,1)}{\left|H^+(1,0)\right|}
      &= 2i\sqrt{\frac{J_y}{J_x}}\conj{f_{1001}} \e{-i\varphi_{y}}
    \label{eq_A01} \\
    B^+_{1,0}&=\frac{V^+(1,0)}{\left|V^+(0,1)\right|}
...
%
\begin{align}
    \left|f_{1001}\right| &= \frac{1}{2}\sqrt{\left| A^+_{0,1}B^+_{1,0} \right|} \notag \\
    \left|f_{1010}\right| &= \frac{1}{2}\sqrt{\left| A^+_{0,-1}B^+_{-1,0} \right|}
...
%
\begin{equation}
    \hat{p}_{z,a} = \frac{\hat{z}_b}{\cos\phi_{ab}} + \hat{z}_a \tan\phi_{ab}
\end{equation}
%
...
%
\begin{equation}
    \frac{H^+(n_x,n_y)_a}{H(1,0)_a} = (1-i\tan\phi_{ab})A(n_x,n_y)_a - \frac{i}{\cos\phi_{ab}}A(n_x,n_y)_b
...
\end{equation}
%
depending on the AC dipole frequency $\Qd{z}$, the kick strength $A_\theta$ and the turn number $N$. Figure~\ref{fig_ac_kick} shows
