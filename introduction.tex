...
\end{align}
%
The calibration factors $C_{x/y}$ cancel out if one devides the spectral line by the main line:
%
\begin{align}
    A_{0,n_y} &= \frac{H^+(0,n_y)}{H^+(1,0)} \notag \\
    B_{n_x,0} &= \frac{V^+(n_x,0)}{V^+(0,1)} 
    \fstop
\end{align}
%
...
%
\begin{align}
    A_{0,1}&=\frac{H^+(0,1)}{H^+(1,0)}  &= 2i\sqrt{\frac{I_y}{I_x}}\conj{f_{1001}} \e{i\varphi_{x,0}-i\varphi_{y,0}}
    \label{eq_A01} \\
    B_{1,0}&=\frac{V^+(1,0)}{V^+(0,1)}  &= 2i\sqrt{\frac{I_x}{I_y}}\conj{f_{1001}} \e{i\varphi_{y,0}-i\varphi_{x,0}}
    \label{eq_B10}
\end{align}
which contain $f_{1001}$ and
\begin{align}
    A_{0,-1}&=\frac{H^+(0,-1)}{H^+(1,0)}  &= 2i\sqrt{\frac{I_y}{I_x}}\conj{f_{1010}} \e{i\varphi_{x,0}+i\varphi_{y,0}}
    \label{eq_A0-1} \\
    B_{-1,0}&=\frac{V^+(-1,0)}{V^+(0,1)}  &= 2i\sqrt{\frac{I_x}{I_y}}\conj{f_{1010}} \e{i\varphi_{y,0}+i\varphi_{x,0}}
    \label{eq_B-10}
\end{align}
%
...
%
\begin{align}
    \left|f_{1001}\right| &= \frac{1}{2}\sqrt{A_{0,1}B_{1,0}} \notag \\
    \left|f_{1010}\right| &= \frac{1}{2}\sqrt{A_{0,-1}B_{-1,0}}
    \fstop
\end{align}
%
...
%
\begin{align}
    A(0,n_y)_a &= \frac{H(0,n_y)_a}{H(1,0)_a} = \frac{H(0,n_y)_a}{H(1,0)_b} \notag \\
    B(n_x,0)_a &= \frac{V(n_x,0)_a}{V(0,1)_a} = \frac{V(n_x,0)_a}{V(0,1)_b} 
    \fstop
\end{align}
%
...
\begin{align}
    h_z(s, N) =
        &\tilde{h}_z(s, N) \notag \\
        &+\frac{A_\theta}{4\sin\left( \pi Q_z^-\right)} \e{-2\pi i \Qd{z} + \varphi_{s_ds}+ \pi i Q_z^-(2N+2\Theta(s,s_d)-1)} 
        \notag\\
        &-\frac{A_\theta}{4\sin\left( \pi Q_z^+\right)} \e{2\pi i N\Qd{z} + \varphi_{s_ds}+ \pi i Q_z^+(2\Theta(s,s_d) -1)} 
    \fstop
    \label{eq_driven_simpl}
\end{align}
%
...
\begin{align}
    h_z(s, N) =
        &\tilde{h}_z(s, N) \notag \\
        &+ \frac{A_\theta}{4\sin\left( \pi Q_z^-\right)} \e{-2N\pi i \Qd{z} + \varphi_{s_ds} + Q_z^-\text{sgn}(s-s_d)} 
        \notag \\
        &- \frac{A_\theta}{4\sin\left( \pi Q_z^+\right)} \e{ 2N\pi i \Qd{z} + \varphi_{s_ds} + Q_z^+\text{sgn}(s-s_d)} 
    \fstop
...
%
\begin{equation}
    g_{z,\pm}(s) = \frac{A_\theta}{4\sin(\pi Q_z^\pm)} \e{i\varphi_{s_ds} + Q_z^\pm\text{sgn}(s-s_d)}\notag \\
\end{equation}
%
