...
\cite{Tomas:optics_review, Tomas2012, Persson2017, optics_sls, optics_cornell, optics_isr, nbpm_alba},
...
advances between two or three BPMs \cite{Franchi2010, Miyamoto2010, Castro:316609}
independently from BPM calibration errors \cite{calibration}
. 
The phase advance measured from a Fourier Transform \cite{Guo_phase_meas,Laskar} turn-by-turn data is independent of the amplitude
...
Excitation methods include single kicks and driven oscillation by an AC-dipole \cite{ac-dipole} which
...
Instead model-based fitting methods \cite{linearHuang}
...
\begin{figure}[htbp]
  \centering
    \includegraphics[width=.4\linewidth]{figIntervPhiIj}
...
\begin{figure}
  \centering
  \includegraphics[width=.4\linewidth]{figThetaComb}
\caption{
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{./sim_locality.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=.45\linewidth]{figThetacombijkl}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{sim_no_noise}
...
and after cleaning \cite{svdclean} and harmonic analysis.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{sim_noise}
...
More precise BPMs like DOROS-BPMs \cite{doros} currently installed in the LHC interaction region and a higher excitation
amplitude as well as the acquisition of a higher number of turns
can increase the resolution of the phase measurement. 

\begin{figure}
  \centering
\includegraphics[width=\linewidth]{sim_peak}
...
\begin{equation}
  ...
  \label{eq_F_quad}
\end{equation}
where the RDT $f_{jklm}$ up to first order at the position $i$ is given by 

\begin{equation}
  \label{eq_rdt}
  f_{jklm,a} = \frac{
    -\sum\limits_w h_{w,jklm}\e{i\left[(j-k)\varphi_{x,wa}+(l-m)\varphi_{y,wa}\right]}
  }{
      1- \e{i \left[ (j-k)Q_x + (l-m)Q_y\right]}
  }\fstop
\end{equation}
For normal multipole components $K_{n} $ the coefficient $h_{w,jklm}$ reads:

\begin{equation}
  h_{w,jklm} = \frac{
    -K_{w,n-1} i^{l+m}\left( \beta_{w,x} \right)^{\frac{j+k}{2}} \left( \beta_{w,y}
    \right)^{\frac{l+m}{2}}
  }{
    j!\;k!\;l!\;m!\; 2^{j+k+l+m}
  }
  ...
  \label{eq_h_jklm}
\end{equation}
The complex Courant-Snyder coordinates can be calculated from the normal form coordinates by \cite{Bartolini1997, Bazzani1994}
\begin{align}
